...
train_dataset = dataloader.Defect_Dataset("severstal-steel-defect-detection", "train", 128, 800)
val_dataset = dataloader.Defect_Dataset("severstal-steel-defect-detection", "val", 128, 800)
trainloader = DataLoader(train_dataset, 4, shuffle=True, num_workers=6)
valloader = DataLoader(val_dataset, 4, shuffle=False, num_workers=6)
    ...
    if epoch % 2 == 0:
        ...
        for (img,mask) in tqdm(valloader):
            ...
            if epoch % 10 == 0 and n_vis < 5 and random.random() < 0.1:
                ...
                total_mask = np.zeros([128,800,3],dtype=int)
                ...
                total_gt = np.zeros([128,800,3],dtype=int)
