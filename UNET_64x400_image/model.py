...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        bottleneck1 = F.interpolate(bottleneck1, size=(9,93), mode='bicubic', align_corners=False)
        
        #Decode
        decode_block3 = self.conv_decode3(torch.cat([encode_block3,bottleneck1], 1))
        decode_block3 = F.interpolate(decode_block3, size=(26,194), mode='bicubic', align_corners=False)
        decode_block2 = self.conv_decode2(torch.cat([encode_block2,decode_block3], 1))
        decode_block2 = F.interpolate(decode_block2, size=(60,396), mode='bicubic', align_corners=False)
        final_layer = self.final_layer(torch.cat([encode_block1,decode_block2], 1))
        final_layer = F.interpolate(final_layer, size=(64,400), mode='bicubic', align_corners=False)
...
if __name__ == "__main__":
    ...
    inp = torch.randn(8, 3, 64, 400).cuda()
    out = model(inp)
    print(out.shape)